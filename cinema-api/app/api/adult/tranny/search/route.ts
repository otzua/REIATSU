...
export async function GET(req: NextRequest) {
  try {
    ...
    const searchUrl = `${BASE_URL}search/?q=${encodeURIComponent(q)}&from_videos=${page}`;
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch search results', status: response.status }, { status: response.status });
    }
    ...
    return NextResponse.json({ success: true, query: q, page: parseInt(page), totalResults: results.length, results });
  } catch (error) {
