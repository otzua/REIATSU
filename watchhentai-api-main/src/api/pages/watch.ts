...
const watchCache = new Map<string, { data: WatchPageData; time: number }>();
...
async function handleWatch(c: any) {
  ...
  if (watchCache.has(slug)) {
    const cached = watchCache.get(slug)!;
    if (Date.now() - cached.time < 1000 * 60 * 10) {
      return c.json({
        success: true,
        data: cached.data,
        meta: {
          scrapedAt: new Date(cached.time).toISOString(),
          source: `https://watchhentai.net/videos/${slug}/`,
          cached: true
        }
      });
    }
  ...
  try {
    ...
    watchCache.set(slug, { data, time: Date.now() });
