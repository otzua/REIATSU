...
function parseHTML(html) {
  const data = { menu: [], footer: [], schedule: {}, randomSeries: [], randomMovies: [] };
  ...
  if (randomSeriesMatch) data.randomSeries = extractMediaItems(randomSeriesMatch[1]);
  ...
  if (randomMoviesMatch) data.randomMovies = extractMediaItems(randomMoviesMatch[1]);
...
export default async function handler(request) {
  ...
  try {
    ...
    if (section) {
      const sectionMap = {
        '1': parsedData.menu,         'menu': parsedData.menu,
        '2': parsedData.footer,       'footer': parsedData.footer,
        '3': parsedData.schedule,     'schedule': parsedData.schedule,
        '4': parsedData.randomSeries, 'randomseries': parsedData.randomSeries, 'random-series': parsedData.randomSeries,
        '5': parsedData.randomMovies, 'randommovies': parsedData.randomMovies, 'random-movies': parsedData.randomMovies,
      };
      const sectionData = sectionMap[section.toLowerCase()];
      if (sectionData) {
        return new Response(JSON.stringify({ success: true, section, data: sectionData }), {
